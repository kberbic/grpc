...
export default class GPRCServer {
    ...
    #exec(item){
        return (service, key) => {
            service[key] =  (_, callback) => {
                this.#validation(this.#validations, _.request, this.#proto[item.name].service[key].requestType)
                    .then(() => {
                        ...
                        return this.#executeModules(this.#modules, _)
                            .then((data) => {
                                ...
                                return action(_.request).then(response => {
                                    // TODO Add support for streams
                                    return this.#validation(this.#validations, response, this.#proto[item.name].service[key].responseType)
                                        .then(() => {
    ...
    #next({resolve, reject}, modules, index, req) {
        this.#middleware(modules[index], req, (err, data) => {
            if (err)
                return reject(err);
            ...
            this.#next({resolve, reject}, modules, index++, req);
        });
    }
